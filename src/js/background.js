    ...
    onComplete: function(response) {
      ...
      if (ss.storage.lastpage == latestsumoquestionid) {
        ...
        if (ss.storage.lastpage < latestsumoquestionid && latestsumoquestionid - ss.storage.lastpage < 7) {
          cyclethrough_questions: for (var i = latestsumoquestionid - ss.storage.lastpage; i >= 0; i--) {
        ...
        if (
          alertexisting !== 1 &&
          ((true === true && sumoquestionlocale === 'en-US') ||
            (prefs.watch_es === true && sumoquestionlocale === 'es') ||
            (prefs.watch_pt === true && sumoquestionlocale === 'pt-BR'))
        ) {
          if (
            (prefs.watch_firefox === true && sumoquestionproduct === 'firefox') ||
            (prefs.watch_mobile === true && sumoquestionproduct === 'mobile') ||
            (prefs.watch_thunderbird === true && sumoquestionproduct === 'thunderbird')
          ) {
        ...
        var getUserdetails = Request({
          ...
          onComplete: function(userresponse) {
            ...
            if (userresponse.json.solution_count === 0) {
              ...
              labeltext =
                ss.storage.answer_count +
                ' answers\n' +
                ss.storage.solution_count +
                ' solutions (' +
                ((ss.storage.solution_count / ss.storage.answer_count) * 100).toFixed(1) +
                '%)\n' +
                ss.storage.helpfulness +
                ' helpful votes (' +
                ((ss.storage.helpfulness / ss.storage.answer_count) * 100).toFixed(1) +
                '%)';
            }
    ...
    onComplete: function(involved) {
      ...
      if (ss.storage.lastinvolvedtime == latestsumoinvolvedupdate) {
        ...
        if (ss.storage.lastinvolvedtime < latestsumoinvolvedupdate && latestsumoinvolvedupdate - ss.storage.lastinvolvedtime < 43200000) {
          cyclethrough_questions: for (var i = 7; i >= 0; i--) {
        ...
        var replyContentRequest = Request({
          ...
          onComplete: function(rresponse2) {
            ...
            if (rresponse2.json.creator.username !== ss.storage.username) {
              ...
              } else if (timedelta <= 7200000 >= 3600000) {
                ago = '1 hour';
              } else {
...
function classifyQuestion(sumoquestiontitle, sumoquestioncontent) {
  if (
    RegExp('season|episode|vashikaran|gold medal|astrolog|black magic|husband wife|garcinia|reddit.com|fuck', 'i').test(sumoquestiontitle + sumoquestioncontent) === true ||
    sumoquestiontitle.length < 9 ||
    sumoquestioncontent.length < 9 ||
    getCharacterCount(sumoquestiontitle) < 6
  ) {
